...
The archive contains the following example binaries (normalized names; original file names in backticks):

- `do_write_m20.exe` — example binary (Windows)
- `dump_words.exe` — example binary (Windows)
- `read_f32_range.exe` — example binary (Windows)
- `read_f32_test.exe` — example binary (Windows)
- `simple.exe` — example binary (Windows)
- `write_various_types.exe` — example binary (Windows)
...
- `do_write_m20` — example binary (Linux x86_64)
- `dump_words` — example binary (Linux x86_64)
- `read_f32_range` — example binary (Linux x86_64)
- `read_f32_test` — example binary (Linux x86_64)
- `simple` — example binary (Linux x86_64)
- `write_various_types` — example binary (Linux x86_64)

Note: The CI artifacts include multiple build variants (commit hash suffixes) alongside the plain-named binaries; choose the file matching your platform/arch.
...
- `do_write_m20` — example binary (Linux aarch64)
- `dump_words` — example binary (Linux aarch64)
- `read_f32_range` — example binary (Linux aarch64)
- `read_f32_test` — example binary (Linux aarch64)
- `simple` — example binary (Linux aarch64)
- `write_various_types` — example binary (Linux aarch64)
...
- `read_f32_test` — example binary (macOS aarch64)
- `do_write_m20` — example binary (macOS aarch64)
- `write_various_types` — example binary (macOS aarch64)
- `simple` — example binary (macOS aarch64)
- `dump_words` — example binary (macOS aarch64)
- `read_f32_range` — example binary (macOS aarch64)

Note: Filenames may include short commit-hash suffixes for CI-built variants; pick the file without suffix if present, or the suffixed file matching your platform.
